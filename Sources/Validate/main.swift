//
//  main.swift
//  mlx-restormer-swift / RestormerValidate
//
...
//
//  Usage:  swift run restormer-validate <weights.safetensors> [image.png] [width] [height]
//
...
guard let weightsPath = args.first else {
    print("usage: restormer-validate <weights.safetensors> [image.png] [width] [height]")
    exit(2)
}
...
func main() async {
    ...
    let config = RestormerConfiguration(weightsURL: URL(fileURLWithPath: weightsPath))
    let request = ImageRestoreRequest(image: makeImage())

    do {
        let result = try await ValidationHarness.run(
            engine: engine,
            registration: RestormerRestorePackage.registration,
            ...
            heartbeatLabel: "restormer")

        print("")
        print(result.run.splitLogLine("restormer-\(config.variant.rawValue)"))
        print("")
